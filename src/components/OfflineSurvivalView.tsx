...
export const OfflineSurvivalView: React.FC = () => {
  ...
  const [packingItems, setPackingItems] = useState<PackingItem[]>(INITIAL_PACKING_LIST);
  ...
  const convertedValue = ((parseFloat(calcAmount) || 0) / exchangeRates[fromCurrency]) * exchangeRates[toCurrency];

  const currentEmergency: EmergencyInfo = EMERGENCY_DB[selectedCountry] || EMERGENCY_DB['Japan'];
  ...
  const completionPercent = Math.round((completedCount / packingItems.length) * 100);
    ...
    <div className="space-y-6">
      ...
      <div className="p-5 rounded-3xl bg-slate-900 border border-slate-800 shadow-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-extrabold text-white">Offline Survival & Emergency Nav</h2>
            <p className="text-xs text-slate-400">Critical local emergency dial codes, embassy contacts & cached tools.</p>
          </div>
        </div>

        {/* Country Selector */}
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-slate-400">Target Region:</span>
          <select
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="p-4 rounded-2xl bg-rose-950/30 border border-rose-900/50 shadow-lg">
          <div className="flex items-center gap-2 text-rose-400 text-xs font-bold uppercase mb-1">
            <ShieldAlert className="w-4 h-4" />
            <span>Police</span>
          </div>
          <div className="text-3xl font-black text-rose-200 font-mono">{currentEmergency.police}</div>
          <span className="text-[11px] text-slate-400 mt-1 block">Local emergency dispatch</span>
        </div>

        <div className="p-4 rounded-2xl bg-amber-950/30 border border-amber-900/50 shadow-lg">
          <div className="flex items-center gap-2 text-amber-400 text-xs font-bold uppercase mb-1">
            <Hospital className="w-4 h-4" />
            <span>Ambulance</span>
          </div>
          <div className="text-3xl font-black text-amber-200 font-mono">{currentEmergency.ambulance}</div>
          <span className="text-[11px] text-slate-400 mt-1 block">Medical response</span>
        </div>

        <div className="p-4 rounded-2xl bg-orange-950/30 border border-orange-900/50 shadow-lg">
          <div className="flex items-center gap-2 text-orange-400 text-xs font-bold uppercase mb-1">
            <AlertTriangle className="w-4 h-4" />
            <span>Fire Rescue</span>
          </div>
          <div className="text-3xl font-black text-orange-200 font-mono">{currentEmergency.fire}</div>
          <span className="text-[11px] text-slate-400 mt-1 block">Fire & rescue squads</span>
        </div>

        <div className="p-4 rounded-2xl bg-cyan-950/30 border border-cyan-900/50 shadow-lg">
          <div className="flex items-center gap-2 text-cyan-400 text-xs font-bold uppercase mb-1">
            <PhoneCall className="w-4 h-4" />
            <span>SOS Universal</span>
          </div>
          <div className="text-3xl font-black text-cyan-200 font-mono">{currentEmergency.emergencyGeneral}</div>
          <span className="text-[11px] text-slate-400 mt-1 block">General dispatch line</span>
        </div>
      </div>
      ...
      <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-3">
        <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
          ...
          <span>Recommended Medical Facility & Consular Access</span>
        </h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
          <div className="p-3.5 rounded-xl bg-slate-800/60 border border-slate-700/60">
            <span className="text-slate-400 block mb-1 font-semibold">Hospital Referral:</span>
            <span className="text-slate-200 font-medium">{currentEmergency.hospital}</span>
          </div>
          <div className="p-3.5 rounded-xl bg-slate-800/60 border border-slate-700/60">
            <span className="text-slate-400 block mb-1 font-semibold">Embassy Assistance:</span>
            <span className="text-slate-200 font-medium">{currentEmergency.embassySupport}</span>
          </div>
        </div>
        <p className="text-xs text-amber-300/90 italic bg-amber-950/20 p-3 rounded-xl border border-amber-900/30">
          💡 Survival Tip: {currentEmergency.tip}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 p-6 rounded-3xl bg-slate-900 border border-slate-800 space-y-4 shadow-xl">
          <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
            ...
            <span>Cached Offline Currency Exchange</span>
          </h3>
          ...
          <div className="space-y-3">
            ...
            <div className="p-4 rounded-2xl bg-emerald-950/30 border border-emerald-900/40 text-center">
        ...
        <div className="lg:col-span-7 p-6 rounded-3xl bg-slate-900 border border-slate-800 space-y-4 shadow-xl">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
                ...
                <span>Expedition Packing Checklist</span>
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">
                {completedCount} of {packingItems.length} packed ({completionPercent}%)
              </p>
            </div>
          ...
          <form onSubmit={handleAddPackingItem} className="flex gap-2">
            <input
              ...
              className="flex-1 px-3.5 py-2 rounded-xl bg-slate-800 border border-slate-700 text-xs text-white focus:outline-none focus:border-cyan-500"
            />
            <button
              type="submit"
            ...
            {packingItems.map(item => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-xs font-medium ${item.checked ? 'line-through text-slate-500' : 'text-slate-200'}`}>
                    {item.name}
                  </span>
                </div>
